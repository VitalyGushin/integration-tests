...
func (s *Suite) TestWebhook_smartvf() {
	...
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=5m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/VitalyGushin/deployments-k8s/examples/features/webhook-smartvf?ref=5c71dab52788c4f7faffd8ae7f9d77ce30832b00`)
	r.Run(`kubectl wait --for=condition=ready --timeout=5m pod -l app=nse-kernel -n ns-webhook-smartvf`)
	r.Run(`kubectl wait --for=condition=ready --timeout=5m pod postgres-cl -n ns-webhook-smartvf`)
